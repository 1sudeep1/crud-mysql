...
import authData from './authRoute'
import { useDispatch, useSelector } from 'react-redux'
import { setSignOut } from '../../redux/reducerSlices/userSlice'
            ...
            <div className="flex flex-wrap items-center justify-between gap-5 w-full">
                ...
                <div className="flex max-lg:ml-auto space-x-3">
                    ...
                    {loggedInUser.isLoggedIn === false ?
                        authData.map((authItem, authId) => {
                            return (
                                <Link key={authId} href={authItem.href} className="px-4 py-2 text-sm rounded-full font-bold text-white border-2 border-[#007bff] bg-[#007bff] transition-all ease-in-out duration-300 hover:bg-transparent hover:text-[#007bff]">{authItem.authTitle}</Link>
                            )
                        })
                        :
